...
<div class="hymain">
    ...
    <script src="/skin/member/dist/js/listdds.min.js"></script>
                ...
                <tr class="order-body" data-cpid = {{ subItem.titleid }}>
                    ...
                    <!--type == 0,买家未付款,订单正常-->
                        ...
                        <td class="order-body-item order-Btn " rowspan="{{item.products.length}}">
                            <p onclick="closedd({{item.ddid}})" class="go-pay-order order-com-button go-send">关闭订单</p>
                        </td>
                    {{#  }else if(item.type== 1){ }}
                    <!--type == 1,买家未付款,订单已取消-->
                        <!--第五列：订单状态-->
...
<!--                            <p onclick="rengongkefu({{item.ddid}})" class="go-pay-order order-com-button go-send">申请人工介入</p>-->
  ...
  <div class="block">
    ...
    <div class="fr rmain">
      <h3>买家订单</h3>
       <div class="tips">
           我的订单提醒：待提交<span>（<?=$dtjnum?>）</span> 待收货<span>（<?=$dshnum?>）</span> 待退货<span>（<?=$dthnum?>）</span> 已完成<span>（<?=$ywcnum?>）</span>
       </div>
